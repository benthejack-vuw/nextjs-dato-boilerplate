...
import RenderMarkdown, { type Components } from "react-markdown";
...
const Strong = ({ children }: PropsWithChildren) => <strong className="font-bold">{children}</strong>;

const Emphasis = ({ children }: PropsWithChildren) => <em className="italic">{children}</em>;

const Strikethrough = ({ children }: PropsWithChildren) => <s className="line-through">{children}</s>;
...
const anchor = ({ children, href }: ComponentProps<"a">) => {
	...
	const params = Object.fromEntries(new URLSearchParams(urlParts[1] ?? "").entries());
...
const markdownComponents: Components = {
	...
	s: Strikethrough,
	del: Strikethrough,
	blockquote: ({ children }: PropsWithChildren) => <blockquote className="">{children}</blockquote>,
	hr: () => <hr className="" />,
...
const Markdown = ({ className, children }: RenderMarkdownProps) => {
	return (
		<div className={className}>
			<RenderMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>
				{children as string}
			</RenderMarkdown>
		</div>
	);

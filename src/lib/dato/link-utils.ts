...
export type InternalLinkBlock = NonNullable<
	InternalLinkFragment | OverridesInternalLinkFragment
>;
export type LinkBlockWithOverrides = NonNullable<
	ExternalLinkFragment | OverridesInternalLinkFragment
>;
...
const internalLinkTypenames = [
	"InternalLinkRecord",
	"OverridesInternalLinkRecord",
];

const externalLinkTypenames = ["ExternalLinkRecord"];

const linkTypesWithManualData = [
	"OverridesInternalLinkRecord",
	"ExternalLinkRecord",
];

const allLinkTypenames = [
	"LinkRecord",
	"InternalLinkRecord",
	"OverridesInternalLinkRecord",
	"ExternalLinkRecord",
...
export const isLinkFragment = (link?: LinkItem): link is LinkFragment =>
	link?.__typename === "LinkRecord";

export const isInternalLinkBlock = (
	link?: LinkItem,
): link is InternalLinkBlock => {
	if (isLinkFragment(link) && isLinkBlock(link.link))
		return isInternalLinkBlock(link.link);

	return !!link && internalLinkTypenames.includes(link.__typename);
};

export const isExternalLinkBlock = (
	link?: LinkItem,
): link is ExternalLinkBlock => {
	if (isLinkFragment(link) && isLinkBlock(link.link))
		return isExternalLinkBlock(link.link);

	return !!link && externalLinkTypenames.includes(link.__typename);
};

export const isInternalLinkTarget = (
	link?: LinkItem,
): link is InternalLinkTarget =>
...
export const linkImage = (
	link: LinkItem,
): LinkMediaAssetFragment | null | undefined => {
	if (isLinkFragment(link) && isLinkBlock(link.link))
		return linkImage(link.link);
